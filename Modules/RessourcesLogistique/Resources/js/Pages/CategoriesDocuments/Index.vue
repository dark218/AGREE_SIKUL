...
const search = () => {
    router.get(route('categories-documents.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const resetFilters = () => {
    ...
    router.get(route('categories-documents.index'));
};
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('categories-documents.statut', itemToDelete.value.id), {
            method: 'put',
            preserveScroll: true,
            onSuccess: () => {
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('categories-documents.destroy', itemToDelete.value.id), {
            method: 'delete',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
...
<template>
    <Head :title="t('common.categorie') || 'Catégorie Document'" />
    <div class="body-wrapper">
        <div class="table-area">
            <!-- Header -->
            <div class="dashboard-header-wrapper">
                <h4 class="title">{{ t('common.categorie') || 'Catégorie Document' }}</h4>
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn">
                        <Link :href="route('categories-documents.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="categoriesDocuments?.data && categoriesDocuments?.data.length > 0">
                                        <tr v-for="categorie in categoriesDocuments?.data" :key="categorie.id">
                                            ...
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('categories-documents.show', categorie.id)" class="btn btn-secondary" :title="t('actions.view')">
                                                        <span class="fa fa-eye"></span>
                                                    </Link>
                                                    <Link :href="route('categories-documents.edit', categorie.id)" class="btn btn-primary" :title="t('actions.edit')">
                                                        <span class="fa fa-edit"></span>
                                                    </Link>
                                                    <button @click="confirmDelete(categorie)" class="btn btn-danger" :title="t('actions.delete')">
                                                        <span class="fa fa-trash"></span>
                                                    </button>
                                                    <button v-if="categorie.statut === 'actif'" @click="categorie.statut === 'actif' ? confirmDeactivate(categorie) : confirmActivate(categorie)" class="btn btn-warning" :title="t('actions.deactivate')">
                                                        <span class="fa fa-ban"></span>
                                                    </button>
                                                    <button v-else @click="categorie.statut === 'actif' ? confirmDeactivate(categorie) : confirmActivate(categorie)" class="btn btn-success" :title="t('actions.activate')">
                        ...
                        <div class="mobile-card-list">
                            <template v-if="categoriesDocuments?.data && categoriesDocuments?.data.length > 0">
                                <div v-for="categorie in categoriesDocuments?.data" :key="'m-' + categorie.id" class="mobile-card">
                                    ...
                                    <div class="mobile-card-actions">
                                        <Link :href="route('categories-documents.show', categorie.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link :href="route('categories-documents.edit', categorie.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                                        <button @click="confirmDelete(categorie)" class="btn btn-danger"><span class="fa fa-trash"></span></button>
                                        <button v-if="categorie.statut === 'actif'" @click="categorie.statut === 'actif' ? confirmDeactivate(categorie) : confirmActivate(categorie)" class="btn btn-warning"><span class="fa fa-ban"></span></button>
                                        <button v-else @click="categorie.statut === 'actif' ? confirmDeactivate(categorie) : confirmActivate(categorie)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                    </div>
                                </div>
                            </template>
                        ...
                        <!-- Pagination -->
                        <Pagination :data="categoriesDocuments" />
                    </div>
                </div>
            </div>
        </div>
