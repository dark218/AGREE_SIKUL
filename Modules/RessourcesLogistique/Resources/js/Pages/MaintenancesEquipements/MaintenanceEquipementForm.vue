<template>
    <form @submit.prevent="submit" class="maintenance-equipement-form">
        ...
        <div v-if="!isReadOnly" class="form-actions mt-4">
            ...
            <Link :href="route('maintenance-equipement.index')" class="btn btn-secondary ms-2">
                {{ t('common.cancel') }}
            </Link>
        </div>
    </form>
...
const form = ref({
    nom: props.maintenanceEquipement?.nom || '',
    code: props.maintenanceEquipement?.code || '',
    statut: props.maintenanceEquipement?.statut || 'actif',
});
function submit() {
