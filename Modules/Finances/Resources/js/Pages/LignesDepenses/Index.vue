...
const search = () => {
    router.get(route('finances.lignes-depenses.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const resetFilters = () => {
  ...
  router.get(route('finances.lignes-depenses.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('finances.lignes-depenses.destroy', itemToDelete.value?.id), {
            method: 'delete',
            preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('finances.lignes-depenses.statut', itemToDelete.value?.id), {
            method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
...
const page = usePage();
const lignes_depenses = props.lignes_depenses || page.props.lignes_depenses;
        ...
        <div class="table-area">
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('lignes-depenses-create')">
                        <Link :href="route('finances.lignes-depenses.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="lignes_depenses?.data && lignes_depenses?.data.length > 0">
                                        <tr v-for="item in lignes_depenses?.data" :key="item.id">
                                            <td>{{ item.code || '' }}</td>
                                            <td>{{ item.libelle || '' }}</td>
                                            ...
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('finances.lignes-depenses.show', item?.id)" class="btn btn-secondary" :title="t('actions.view')">
                                                        <span class="fa fa-eye"></span>
                                                    </Link>
                                                    <Link :href="route('finances.lignes-depenses.edit', item?.id)" class="btn btn-primary" :title="t('actions.edit')">
                        ...
                        <div class="mobile-card-list">
                            <template v-if="lignes_depenses?.data && lignes_depenses?.data.length > 0">
                                <div v-for="item in lignes_depenses?.data" :key="'m-' + item.id" class="mobile-card">
                                    ...
                                    <div class="mobile-card-actions">
                                        <Link :href="route('finances.lignes-depenses.show', item?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link :href="route('finances.lignes-depenses.edit', item?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                        ...
                        <Pagination :data="lignes_depenses" />
                    </div>
                </div>
            </div>
        </div>
