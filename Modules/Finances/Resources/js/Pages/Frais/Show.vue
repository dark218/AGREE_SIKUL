...
function performDelete() {
    ...
    router.put(route('finances.frais.statut', props.item.id), {}, {
        onSuccess: () => {
...
<template>
    ...
    <div class="body-wrapper">
        <div class="dashboard-header-wrapper">
            ...
            <div class="actions">
                <Link v-if="can('frais-edit')" :href="route('finances.frais.edit', item.id)" class="btn btn-warning">
                    {{ t('common.edit') }}
                </Link>
                <button v-if="can('frais-delete')" @click="confirmDelete" class="btn btn-danger">
                    {{ t('common.delete') }}
                </button>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <FraisForm :read-only="true" :frais="item" />
            </div>
        </div>
        <ConfirmModal
