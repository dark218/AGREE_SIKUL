...
const search = () => {
    router.get(route('finances.postes-recettes.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const resetFilters = () => {
  ...
  router.get(route('finances.postes-recettes.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('finances.postes-recettes.destroy', itemToDelete.value?.id), {
            method: 'delete',
            preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('finances.postes-recettes.statut', itemToDelete.value?.id), {
            method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
...
const page = usePage();
const postes_recettes = props.postes_recettes || page.props.postes_recettes;
        ...
        <div class="table-area">
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('postes-recettes-create')">
                        <Link :href="route('finances.postes-recettes.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="postes_recettes?.data && postes_recettes?.data.length > 0">
                                        <tr v-for="item in postes_recettes?.data" :key="item.id">
                                            <td>{{ item.code || '' }}</td>
                                            <td>{{ item.libelle || '' }}</td>
                                            ...
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('finances.postes-recettes.show', item?.id)" class="btn btn-secondary" :title="t('actions.view')">
                                                        <span class="fa fa-eye"></span>
                                                    </Link>
                                                    <Link :href="route('finances.postes-recettes.edit', item?.id)" class="btn btn-primary" :title="t('actions.edit')">
                        ...
                        <div class="mobile-card-list">
                            <template v-if="postes_recettes?.data && postes_recettes?.data.length > 0">
                                <div v-for="item in postes_recettes?.data" :key="'m-' + item.id" class="mobile-card">
                                    ...
                                    <div class="mobile-card-actions">
                                        <Link :href="route('finances.postes-recettes.show', item?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link :href="route('finances.postes-recettes.edit', item?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                        ...
                        <Pagination :data="postes_recettes" />
                    </div>
                </div>
            </div>
        </div>
