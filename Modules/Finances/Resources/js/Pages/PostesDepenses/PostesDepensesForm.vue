...
<template>
    <div class="row g-3 custom-input">
        ...
        <div class="col-sm-6">
            <div class="mb-3">
                <label>{{ t('fields.ligne_depense_id') || 'Ligne Dépense' }} <span class="text-danger">*</span></label>
                <SearchableSelect
                    v-model="form.ligne_depense_id"
                    :options="lignes_depenses"
                    optionValue="id"
                    optionLabel="libelle"
                    :placeholder="t('fields.ligne_depense_id') || 'Sélectionner Ligne Dépense'"
                    :disabled="isReadOnly"
                />
                <span v-if="form.errors?.ligne_depense_id" class="text-danger">
                    <strong>{{ form.errors.ligne_depense_id }}</strong>
                </span>
            </div>
        </div>
