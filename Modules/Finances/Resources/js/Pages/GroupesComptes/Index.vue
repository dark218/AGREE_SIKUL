...
const searchFilters = ref({
    code_groupe: props.filters?.code_groupe || '',
    libelle_groupes: props.filters?.libelle_groupes || '',
...
const search = () => {
    router.get(route('finances.groupes-comptes.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const resetFilters = () => {
  ...
  router.get(route('finances.groupes-comptes.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('finances.groupes-comptes.destroy', itemToDelete.value?.id), {
            method: 'delete',
            preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('finances.groupes-comptes.statut', itemToDelete.value?.id), {
            method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
...
const page = usePage();
// Make groupes_comptes accessible in template from props or page
const groupes_comptes = props.groupes_comptes || page.props.groupes_comptes;
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('groupes-comptes-create')">
                        <Link :href="route('finances.groupes-comptes.create')" class="btn btn-primary">
                ...
                <div class="card-body">
                    <div class="table-wrapper">
                            ...
                            <table class="custom-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ t('fields.nombre_comptes') || 'Nombre Comptes' }}</th>
                                        <th>{{ t('fields.status') || 'Statut' }}</th>
                                        <th class="fit">{{ t('common.actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <template v-if="groupes_comptes?.data && groupes_comptes?.data.length > 0">
                                        <tr v-for="item in groupes_comptes?.data" :key="item.id">
                                            <td>{{ item.code_groupe || '' }}</td>
                                            <td>{{ item.libelle_groupes || '' }}</td>
                                            <td>{{ item.nombre_comptes || '' }}</td>
                                            <td><span class="badge" :class="item.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + item.etat) }}</span></td>
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('finances.groupes-comptes.show', item?.id)" class="btn btn-secondary" :title="t('actions.view')">
                                                        <span class="fa fa-eye"></span>
                                                    </Link>
                                                    <Link :href="route('finances.groupes-comptes.edit', item?.id)" class="btn btn-primary" :title="t('actions.edit')">
                        ...
                        <div class="mobile-card-list">
                            <template v-if="groupes_comptes?.data && groupes_comptes?.data.length > 0">
                                <div v-for="item in groupes_comptes?.data" :key="'m-' + item.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.code') || 'Code' }}</span>
                                            <span class="mobile-card-value">{{ item.code_groupe || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.label') || 'Libellé' }}</span>
                                            <span class="mobile-card-value">{{ item.libelle_groupes || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.nombre_comptes') || 'Nombre Comptes' }}</span>
                                            <span class="mobile-card-value">{{ item.nombre_comptes || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                    ...
                                    <div class="mobile-card-actions">
                                        <Link :href="route('finances.groupes-comptes.show', item?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link :href="route('finances.groupes-comptes.edit', item?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                        ...
                        <!-- Pagination -->
                        <Pagination :data="groupes_comptes" />
                    </div>
                </div>
            </div>
        </div>
