<template>
    <form @submit.prevent="submit" class="echeancier-form">
        ...
        <div v-if="!isReadOnly" class="form-actions mt-4">
            ...
            <Link :href="route('finances.echeancier.index')" class="btn btn-secondary ms-2">
                {{ t('common.cancel') }}
            </Link>
        </div>
    </form>
...
const form = ref({
    nom: props.echeancier?.nom || '',
    code: props.echeancier?.code || '',
    statut: props.echeancier?.statut || 'actif',
});
function submit() {
