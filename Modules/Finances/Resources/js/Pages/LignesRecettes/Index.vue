...
const search = () => {
    router.get(route('finances.lignes-recettes.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const resetFilters = () => {
  ...
  router.get(route('finances.lignes-recettes.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('finances.lignes-recettes.destroy', itemToDelete.value?.id), {
            method: 'delete',
            preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('finances.lignes-recettes.statut', itemToDelete.value?.id), {
            method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
...
const page = usePage();
const lignes_recettes = props.lignes_recettes || page.props.lignes_recettes;
        ...
        <div class="table-area">
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('lignes-recettes-create')">
                        <Link :href="route('finances.lignes-recettes.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="lignes_recettes?.data && lignes_recettes?.data.length > 0">
                                        <tr v-for="item in lignes_recettes?.data" :key="item.id">
                                            <td>{{ item.code || '' }}</td>
                                            <td>{{ item.libelle || '' }}</td>
                                            ...
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('finances.lignes-recettes.show', item?.id)" class="btn btn-secondary" :title="t('actions.view')">
                                                        <span class="fa fa-eye"></span>
                                                    </Link>
                                                    <Link :href="route('finances.lignes-recettes.edit', item?.id)" class="btn btn-primary" :title="t('actions.edit')">
                        ...
                        <div class="mobile-card-list">
                            <template v-if="lignes_recettes?.data && lignes_recettes?.data.length > 0">
                                <div v-for="item in lignes_recettes?.data" :key="'m-' + item.id" class="mobile-card">
                                    ...
                                    <div class="mobile-card-actions">
                                        <Link :href="route('finances.lignes-recettes.show', item?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link :href="route('finances.lignes-recettes.edit', item?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                        ...
                        <Pagination :data="lignes_recettes" />
                    </div>
                </div>
            </div>
        </div>
