    ...
    <div class="body-wrapper">
        ...
        <form @submit.prevent="search" class="filter-form row mb-3">
            ...
            <div class="col-md-3" v-if="filters.search || filters.statut">
                <Link :href="route('finances.depense.index')" class="btn btn-outline-secondary btn-block">
                    {{ t('common.reset') }}
                </Link>
            </div>
        </form>
        <div class="table-responsive">
            <table class="custom-table">
                ...
                <tbody>
                    <tr v-for="depense in depenses.data" :key="depense.id">
                        <td><strong>{{ depense.nom || depense.name || depense.titre }}</strong></td>
                        <td>
                            <span :class="['badge', getStatutBadgeClass(depense.statut)]">
                                {{ getStatutLabel(depense.statut) }}
                            </span>
                        </td>
                        <td class="fit">
            ...
            <div v-if="!depenses.data.length" class="alert alert-info text-center">
                {{ t('common.no_data') }}
            </div>
        </div>
        <Pagination :data="depenses" :preserve-scroll="true" />
        <ConfirmModal
            :show="showDeleteModal"
            :title="t('common.confirm_delete')"
            :message="depenseToDelete?.etat === 'actif' || depenseToDelete?.statut === 'actif' ? t('messages.confirm.deactivate.message') : t('messages.confirm.activate.message')"
