<script setup>
import { ref, computed } from 'vue';
import { Link } from '@inertiajs/vue3';
import { useI18n } from 'vue-i18n';
import DashboardLayout from '@/Layouts/DashboardLayout.vue';
import AlertMessage from '@/Components/Common/AlertMessage.vue';
import ArticleForm from './ArticleForm.vue';
defineOptions({ layout: DashboardLayout });
const { t } = useI18n();
const props = defineProps({
    article: Object,
    devise: Array,
});
const normalizePrix = (prix) => {
    if (!prix) return 0;
    // Convertir en string et remplacer virgule par point, puis enlever les espaces
    const normalized = String(prix).replace(/,/g, '.').replace(/\s/g, '');
    return parseFloat(normalized) || 0;
};
const form = {
    points_vente_id: props.article.points_vente_id || '',
    sku: props.article.sku || '',
    nom: props.article.nom || '',
    marque: props.article.marque || '',
    description: props.article.description || '',
    prix_cents: normalizePrix(props.article.prix_display),
    devise: props.article.devise || 'XOF',
    quantite_stock: props.article.quantite_stock || 0,
    seuil_alert_stock: props.article.seuil_alert_stock || 0,
    taxes_json: props.article.taxes_json || null,
    errors: {},
};
const pointsVente = computed(() => {
    if (props.article.point_vente) {
        return [{ value: props.article.points_vente_id, label: props.article.point_vente.nom }];
    }
    return [];
});
const isCollapsed = ref(false);
const toggleCollapse = () => {
    isCollapsed.value = !isCollapsed.value;
};
const getStockBadgeClass = () => {
    const article = props.article;
    if (article.quantite_stock <= 0) {
        return 'bg-danger';
    } else if (article.quantite_stock <= article.seuil_alert_stock) {
        return 'bg-warning';
    }
    return 'bg-success';
};
const getStockLabel = () => {
    const article = props.article;
    if (article.quantite_stock <= 0) {
        return t('modules.business.articles.stockEmpty');
    } else if (article.quantite_stock <= article.seuil_alert_stock) {
        return t('modules.business.articles.stockLow');
    }
    return t('modules.business.articles.stockOk');
};
</script>
<template>
    <div class="body-wrapper">
        <div class="row justify-content-center mb-30-none">
            <div class="col-xl-12 mb-30">
                <div class="dash-payment-item-wrapper">
                    <div class="dash-payment-item active">
                        <div class="dash-payment-title-area d-flex justify-content-between align-items-center" @click="toggleCollapse">
                            <div class="d-flex align-items-center">
                                <span class="dash-payment-badge">!</span>
                                <h5 class="title mb-0">{{ t('modules.business.articles.show') }}</h5>
                            </div>
                            <div class="d-flex flex-grow-1 align-items-center justify-content-end">
                                <!-- Statut Stock à droite -->
                                <div class="d-flex align-items-center gap-1">
                                    <small class="text-muted">{{ t('modules.business.articles.stockStatus') }}:</small>
                                    <span :class="['badge', getStockBadgeClass()]">
                                        {{ getStockLabel() }}
                                    </span>
                                </div>
                                <!-- Bouton de réduction -->
                                <button type="button" class="collapse-toggle ms-3" :class="{ collapsed: isCollapsed }" @click.stop="toggleCollapse">
                                    <i class="fa fa-chevron-up"></i>
                                </button>
                            </div>
                        </div>
                        <div class="dash-payment-body" :class="{ collapsed: isCollapsed }">
                            <AlertMessage />
                            <ArticleForm
                                :form="form"
                                :pointsVente="pointsVente"
                                :devise="devise"
                                mode="show"
                            />
                            <div class="row mt-3">
                                <div class="col">
                                    <div class="text-end">
                                        <Link :href="route('article.index')" class="btn btn-danger">
                                            <i class="fa fa-arrow-left"></i> {{ t('actions.back') }}
                                        </Link>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>
