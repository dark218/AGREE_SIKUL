...
const validerKyc = () => {
    showValidateLoader();
    router.post(route('agent.kyc.validation', [props.agent.uuid, 'valider']), {}, {
        preserveScroll: true,
...
const rejeterKyc = (motif) => {
    showRejectLoader();
    router.post(route('agent.kyc.validation', [props.agent.uuid, 'rejeter']), { motif }, {
        preserveScroll: true,
...
const validerStatut = () => {
    showValidateLoader();
    router.post(route('agent.validation', [props.agent.uuid, 'valider']), {}, {
        preserveScroll: true,
...
const rejeterStatut = (motif) => {
    showRejectLoader();
    router.post(route('agent.validation', [props.agent.uuid, 'rejeter']), { motif }, {
        preserveScroll: true,
...
<template>
    <Head :title="t('modules.personnel.agent.show')" />
                    ...
                    <div class="dash-payment-item active">
                        <div class="dash-payment-title-area d-flex justify-content-between align-items-center" @click="toggleCollapse">
                            <div class="d-flex align-items-center">
                                ...
                                <h5 class="title mb-0">{{ t('modules.personnel.agent.show') }}</h5>
                            </div>
                            <div class="d-flex flex-grow-1 align-items-center">
                                ...
                                <div class="d-flex align-items-center gap-3">
                                    <div class="d-flex align-items-center gap-1">
                                        ...
                                        <span :class="['badge', getKycBadgeClass(agent?.kyc_status)]">{{ getKycLabel(agent?.kyc_status) }}</span>
                                    </div>
                                    <div class="d-flex align-items-center gap-1">
                                        <small class="text-muted">{{ t('common.status') }}:</small>
                                        <span :class="['badge', getStatutBadgeClass(agent?.statut)]">{{ getStatutLabel(agent?.statut) }}</span>
                                    </div>
                                </div>
                        ...
                        <div class="dash-payment-body" :class="{ collapsed: isCollapsed }">
                            <div class="col-xl-12 col-lg-6 mb-20">
                                <AlertMessage />
                                <PersonnelForm :user="agent" :payss="payss" :type-pieces="typePieces" :show-pays-field="true" :is-read-only="true" />
                                <div class="row mt-4">
                                    <div class="col">
                                        <div class="text-end">
                                            <Link :href="route('agent.index')" class="btn btn-danger"><i class="fa fa-arrow-left"></i> {{ t('actions.back') }}</Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
