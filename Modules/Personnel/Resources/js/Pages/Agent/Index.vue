...
function deleteItem() {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.put(route('agent.statut', itemToDelete.value.uuid), {}, {
...
function validerItem() {
    if (itemToValidate.value) {
        showValidateLoader();
        router.post(route('agent.validation', [itemToValidate.value.uuid, 'valider']), {}, {
            preserveScroll: true,
            onSuccess: () => {
...
function rejeterItem(motif) {
    if (itemToReject.value) {
        showRejectLoader();
        router.post(route('agent.validation', [itemToReject.value.uuid, 'rejeter']), { motif }, {
            preserveScroll: true,
            onSuccess: () => {
...
function suspendreItem(motif) {
    if (itemToSuspendre.value) {
        showSuspendLoader();
        router.post(route('agent.suspendre', itemToSuspendre.value.uuid), { motif }, {
            preserveScroll: true,
            onSuccess: () => {
...
function bloquerItem(motif) {
    if (itemToBloquer.value) {
        showBlockLoader();
        router.post(route('agent.bloquer', itemToBloquer.value.uuid), { motif }, {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <Head :title="t('modules.personnel.agent.title')" />
    <div class="body-wrapper">
        <div class="table-area">
            <div class="dashboard-header-wrapper">
                <h4 class="title">{{ t('modules.personnel.agent.title') }}</h4>
                <div class="dashboard-btn-wrapper">
                    <div v-if="can('agent-create')" class="dashboard-btn">
                        <Link :href="route('agent.create')" class="btn btn-primary"><i class="fa fa-plus"></i> {{ t('actions.add') }}</Link>
                    </div>
                </div>
            </div>
            <AlertMessage />
                ...
                <div class="table-wrapper">
                    <div class="table-responsive">
                        <table class="custom-table">
                            ...
                            <tbody>
                                <tr v-if="agent.data && agent.data.length > 0" v-for="item in agent.data" :key="item.id">
                                    <td>{{ item.nom }}</td>
                                    <td>{{ item.prenoms }}</td>
                                    <td>{{ item.login }}</td>
                                    <td>{{ item.pays }}</td>
                                    ...
                                    <td class="fit">
                                        <div class="action-buttons">
                                            <Link :href="route('agent.show', item.uuid)" class="btn btn-secondary btn-sm" :title="t('actions.view')"><i class="fa fa-eye"></i></Link>
                                            <Link v-if="can('agent-edit')" :href="route('agent.edit', item.uuid)" class="btn btn-primary btn-sm" :title="t('actions.edit')"><i class="fa fa-edit"></i></Link>
                                            <button v-if="can('agent-statut')" @click="confirmDelete(item)" class="btn btn-danger btn-sm" :title="t('actions.delete')"><i class="fa fa-trash"></i></button>
                                            <button v-if="can('agent-statut') && canValidateStatut(item)" type="button" class="btn btn-success btn-sm" @click="openValidateModal(item)" :title="t('actions.validate')"><i class="fa fa-check"></i></button>
                                            <button v-if="can('agent-statut') && canValidateStatut(item)" type="button" class="btn btn-warning btn-sm" @click="openRejectModal(item)" :title="t('actions.reject')"><i class="fa fa-times"></i></button>
                                            ...
                                            <Link v-if="can('wallet-show-by-owner')" :href="route('wallet.show-by-owner', { ownerType: 'agent', ownerId: item.id })" class="btn btn-info btn-sm" :title="t('modules.wallet.singular')"><i class="fa fa-wallet"></i></Link>
                                        </div>
                                    </td>
                                </tr>
                        ...
                        <Pagination :data="agent" :preserve-scroll="true" :preserve-state="true" />
                    </div>
                </div>
            </div>
        </div>
