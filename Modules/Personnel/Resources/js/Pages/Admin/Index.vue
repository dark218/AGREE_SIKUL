...
function deleteItem() {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.put(route('admin.statut', itemToDelete.value.uuid), {}, {
            onSuccess: () => {
...
function validerItem() {
    if (itemToValidate.value) {
        showValidateLoader();
        router.post(route('admin.validation', [itemToValidate.value.uuid, 'valider']), {}, {
            preserveScroll: true,
            onSuccess: () => {
...
function rejeterItem(motif) {
    if (itemToReject.value) {
        showRejectLoader();
        router.post(route('admin.validation', [itemToReject.value.uuid, 'rejeter']), { motif }, {
            preserveScroll: true,
            onSuccess: () => {
...
function suspendreItem(motif) {
    if (itemToSuspendre.value) {
        showSuspendLoader();
        router.post(route('admin.suspendre', itemToSuspendre.value.uuid), { motif }, {
            preserveScroll: true,
            onSuccess: () => {
...
function bloquerItem(motif) {
    if (itemToBloquer.value) {
        showBlockLoader();
        router.post(route('admin.bloquer', itemToBloquer.value.uuid), { motif }, {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <Head :title="t('modules.personnel.admin.title')" />
    <div class="body-wrapper">
        <div class="table-area">
            <div class="dashboard-header-wrapper">
                <h4 class="title">{{ t('modules.personnel.admin.title') }}</h4>
                <div class="dashboard-btn-wrapper">
                    <div v-if="can('admin-create')" class="dashboard-btn">
                        <Link :href="route('admin.create')" class="btn btn-primary">
            ...
            <div class="row m-0 mb-3">
                <form class="row col-12" @submit.prevent="search">
                    <div class="col-2 p-1">
                        <input v-model="searchFilters.nom" type="text" class="form-control search-slt"
                            :placeholder="t('fields.name')">
                    </div>
                    <div class="col-2 p-1">
                        <input v-model="searchFilters.login" type="text" class="form-control search-slt"
                            :placeholder="t('fields.login')">
                    </div>
                    <div class="col-2 p-1">
                        <StylishSelect v-model="searchFilters.kyc_status" :options="kycStatusOptions" option-value="id"
                            option-label="label" placeholder="KYC" :searchable="false" />
                    </div>
                    <div class="col-2 p-1">
                        <StylishSelect v-model="searchFilters.statut" :options="statutOptions" option-value="id"
                            option-label="label" :placeholder="t('common.status')" :searchable="false" />
                    </div>
                    <div class="col-2 p-1">
                ...
                <div class="table-wrapper">
                    <div class="table-responsive">
                        <table class="custom-table">
                            ...
                            <tbody>
                                <tr v-if="admin.data && admin.data.length > 0" v-for="item in admin.data" :key="item.id">
                                    <td>{{ item.nom }}</td>
                                    <td>{{ item.prenoms }}</td>
                                    <td>{{ item.login }}</td>
                                    <td>{{ item.pays }}</td>
                                    <td>
                                        <span :class="['badge text-dark rounded-pill', getKycBadgeClass(item.kyc_status)]">
                                            {{ getKycLabel(item.kyc_status) }}
                                        </span>
                                    </td>
                                    <td>
                                        <span :class="['badge text-dark rounded-pill', getStatutBadgeClass(item.statut)]">
                                            {{ getStatutLabel(item.statut) }}
                                        </span>
                                    </td>
                                    <td class="fit">
                                        <div class="action-buttons">
                                            <Link :href="route('admin.show', item.uuid)" class="btn btn-secondary btn-sm"
                                                :title="t('actions.view')">
                                                <i class="fa fa-eye"></i>
                                            </Link>
                                            <Link v-if="can('admin-edit')" :href="route('admin.edit', item.uuid)"
                                                class="btn btn-primary btn-sm" :title="t('actions.edit')">
                                                <i class="fa fa-edit"></i>
                                            </Link>
                                            <button v-if="can('admin-statut')" @click="confirmDelete(item)"
                                                class="btn btn-danger btn-sm" :title="t('actions.delete')">
                                                <i class="fa fa-trash"></i>
                                            </button>
                                            <button v-if="can('admin-statut') && canValidateStatut(item)" type="button"
                                                class="btn btn-success btn-sm" @click="openValidateModal(item)"
                                                :title="t('actions.validate')">
                                                <i class="fa fa-check"></i>
                                            </button>
                                            <button v-if="can('admin-statut') && canValidateStatut(item)" type="button"
                                                class="btn btn-warning btn-sm" @click="openRejectModal(item)"
                                                :title="t('actions.reject')">
                                                <i class="fa fa-times"></i>
                                            </button>
                                            <button v-if="item.statut === 'actif'" type="button"
                                                class="btn btn-warning btn-sm" @click="openSuspendreModal(item)"
                                                :title="t('actions.suspend')">
                                                <i class="fa fa-pause"></i>
                                            </button>
                                            <button v-if="item.statut === 'actif'" type="button"
                                                class="btn btn-danger btn-sm" @click="openBloquerModal(item)"
                        ...
                        <Pagination :data="admin" :preserve-scroll="true" :preserve-state="true" />
                    </div>
                </div>
            </div>
        </div>
        <ConfirmModal :show="showDeleteModal" :title="t('messages.confirm.delete.title')"
            :message="itemToDelete?.etat === 'actif' || itemToDelete?.statut === 'actif' ? t('messages.confirm.deactivate.message') : t('messages.confirm.activate.message')" :sub-message="t('messages.confirm.delete.warning')"
            confirm-text="Supprimer" confirm-class="btn-danger" @confirm="deleteItem" @close="closeDeleteModal" />
        <ConfirmModal :show="showValidateModal" :title="t('messages.confirm.validate.title')"
            ...
            @confirm="validerItem" :confirm-text="t('actions.validate')" confirm-class="btn-success" variant="success" />
        <RejectModal :show="showRejectModal" :title="t('messages.confirm.reject.title')"
            @close="showRejectModal = false" @confirm="rejeterItem" />
        <RejectModal :show="showSuspendreModal" :title="t('actions.suspend')" :message="t('messages.confirm.suspend')"
            ...
            @close="showSuspendreModal = false" @confirm="suspendreItem" />
        <RejectModal :show="showBloquerModal" :title="t('actions.block')" :message="t('messages.confirm.block')"
            ...
            @close="showBloquerModal = false" @confirm="bloquerItem" />
