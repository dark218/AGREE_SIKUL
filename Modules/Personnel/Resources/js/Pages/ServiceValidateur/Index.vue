...
function search() {
    router.get(route('service_validateur.index'), {
...
function deleteItem() {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.put(route('service_validateur.statut', itemToDelete.value.uuid), {}, {
...
function validerItem() {
    if (itemToValidate.value) {
        showValidateLoader();
        router.post(route('service_validateur.validation', [itemToValidate.value.uuid, 'valider']), {}, {
            preserveScroll: true,
            onSuccess: () => {
...
function rejeterItem(motif) {
    if (itemToReject.value) {
        showRejectLoader();
        router.post(route('service_validateur.validation', [itemToReject.value.uuid, 'rejeter']), { motif }, {
            preserveScroll: true,
            onSuccess: () => {
...
function suspendreItem(motif) {
    if (itemToSuspendre.value) {
        showSuspendLoader();
        router.post(route('service_validateur.suspendre', itemToSuspendre.value.uuid), { motif }, {
            preserveScroll: true,
            onSuccess: () => {
...
function bloquerItem(motif) {
    if (itemToBloquer.value) {
        showBlockLoader();
        router.post(route('service_validateur.bloquer', itemToBloquer.value.uuid), { motif }, {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <Head :title="t('modules.personnel.service_validateur.title')" />
    <div class="body-wrapper">
        <div class="table-area">
            <div class="dashboard-header-wrapper">
                <h4 class="title">{{ t('modules.personnel.service_validateur.title') }}</h4>
                <div class="dashboard-btn-wrapper">
                    <div v-if="can('service_validateur-create')" class="dashboard-btn">
                        <Link :href="route('service_validateur.create')" class="btn btn-primary"><i class="fa fa-plus"></i> {{ t('actions.add') }}</Link>
                    </div>
                </div>
            </div>
            <AlertMessage />
                ...
                <div class="table-wrapper">
                    <div class="table-responsive">
                        <table class="custom-table">
                            ...
                            <tbody>
                                <tr v-if="serviceValidateur?.data && serviceValidateur.data.length > 0" v-for="item in serviceValidateur.data" :key="item.id">
                                    <td>{{ item.nom }}</td>
                                    <td>{{ item.prenoms }}</td>
                                    <td>{{ item.login }}</td>
                                    <td>{{ item.pays }}</td>
                                    ...
                                    <td class="fit">
                                        <div class="action-buttons">
                                            <Link :href="route('service_validateur.show', item.uuid)" class="btn btn-secondary btn-sm" :title="t('actions.view')"><i class="fa fa-eye"></i></Link>
                                            <Link v-if="can('service_validateur-edit')" :href="route('service_validateur.edit', item.uuid)" class="btn btn-primary btn-sm" :title="t('actions.edit')"><i class="fa fa-edit"></i></Link>
                                            <button v-if="can('service_validateur-statut')" @click="confirmDelete(item)" class="btn btn-danger btn-sm" :title="t('actions.delete')"><i class="fa fa-trash"></i></button>
                                            <button v-if="can('service_validateur-statut') && canValidateStatut(item)" type="button" class="btn btn-success btn-sm" @click="openValidateModal(item)" :title="t('actions.validate')"><i class="fa fa-check"></i></button>
                                            <button v-if="can('service_validateur-statut') && canValidateStatut(item)" type="button" class="btn btn-warning btn-sm" @click="openRejectModal(item)" :title="t('actions.reject')"><i class="fa fa-times"></i></button>
                        ...
                        <Pagination :data="serviceValidateur" :preserve-scroll="true" :preserve-state="true" />
                    </div>
                </div>
            </div>
        </div>
