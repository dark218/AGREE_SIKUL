...
function performDelete() {
    ...
    router.put(route('rapport.statut', props.item.id), {}, {
        onSuccess: () => {
...
<template>
    ...
    <div class="body-wrapper">
        <div class="dashboard-header-wrapper">
            ...
            <div class="actions">
                <Link v-if="can('rapport-edit')" :href="route('rapport.edit', item.id)" class="btn btn-warning">
                    {{ t('common.edit') }}
                </Link>
                <button v-if="can('rapport-delete')" @click="confirmDelete" class="btn btn-danger">
                    {{ t('common.delete') }}
                </button>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <RapportForm :read-only="true" :rapport="item" />
            </div>
        </div>
        <ConfirmModal
