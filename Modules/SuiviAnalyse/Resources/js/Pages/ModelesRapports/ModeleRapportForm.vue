<template>
    <form @submit.prevent="submit" class="modele-rapport-form">
        ...
        <div v-if="!isReadOnly" class="form-actions mt-4">
            ...
            <Link :href="route('modele-rapport.index')" class="btn btn-secondary ms-2">
                {{ t('common.cancel') }}
            </Link>
        </div>
    </form>
...
const form = ref({
    nom: props.modeleRapport?.nom || '',
    code: props.modeleRapport?.code || '',
    statut: props.modeleRapport?.statut || 'actif',
});
function submit() {
