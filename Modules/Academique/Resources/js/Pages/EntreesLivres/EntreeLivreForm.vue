...
<template>
    <div class="row g-3 custom-input">
        ...
        <!-- Infos livre (auto, lecture seule) -->
        ...
        <div class="col-sm-4">
            <div class="mb-3">
                <label>{{ t('fields.entry_type') || "Type d'entrée" }} <span v-if="!isReadOnly" class="text-danger">*</span></label>
                <SearchableSelect v-model="form.type_entree" :options="typeOptions" optionValue="id" optionLabel="libelle" :placeholder="t('actions.select') || '-- Sélectionner --'" :disabled="isReadOnly" />
                <span v-if="form.errors?.type_entree" class="text-danger"><strong>{{ form.errors.type_entree }}</strong></span>
            </div>
        </div>
        <div class="col-sm-4">
            <div class="mb-3">
                <label>{{ t('fields.entry_date') || "Date d'entrée" }}</label>
                <input v-model="form.date_entree" type="date" class="form-control" :disabled="isReadOnly" />
                <span v-if="form.errors?.date_entree" class="text-danger"><strong>{{ form.errors.date_entree }}</strong></span>
            </div>
        </div>
        <div class="col-sm-4">
            <div class="mb-3">
        ...
        <div class="col-sm-4">
            <div class="mb-3">
                <label>{{ t('fields.provider') || 'Prêteur / Vendeur / Donateur' }}</label>
