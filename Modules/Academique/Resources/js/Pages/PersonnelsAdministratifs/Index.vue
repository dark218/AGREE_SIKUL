...
const search = () => {
    router.get(route('academique.personnels_administratifs.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const resetFilters = () => {
    ...
    router.get(route('academique.personnels_administratifs.index'));
};
...
const deleteItem = () => {
    if (selectedItem.value) {
        showDeleteLoader();
        router.visit(route('academique.personnels_administratifs.destroy', selectedItem.value.id), {
            method: 'delete',
            preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = (item) => {
    if (item) {
        showDeleteLoader();
        router.visit(route('academique.personnels_administratifs.statut', item.id), {
            method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                selectedItem.value = null;
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('personnels-administratifs-create')">
                        <Link :href="route('academique.personnels_administratifs.create')" class="btn btn-primary">
                                        ...
                                        <tr v-for="item in personnelsAdministratifs?.data" :key="item.id">
                                            ...
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('academique.personnels_administratifs.show', item.id)" class="btn btn-secondary btn-sm" :title="t('actions.view')">
                                                        <span class="fa fa-eye"></span>
                                                    </Link>
                                                    <Link :href="route('academique.personnels_administratifs.edit', item.id)" class="btn btn-primary btn-sm" :title="t('actions.edit')">
                            ...
                            <template v-if="personnelsAdministratifs?.data && personnelsAdministratifs?.data.length > 0">
                                <div v-for="item in personnelsAdministratifs?.data" :key="'m-' + item.id" class="mobile-card">
                                    ...
                                    <div class="mobile-card-actions">
                                        <Link :href="route('academique.personnels_administratifs.show', item.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link :href="route('academique.personnels_administratifs.edit', item.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
