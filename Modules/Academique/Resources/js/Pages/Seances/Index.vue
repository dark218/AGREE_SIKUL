...
const search = () => {
    router.get(route('academique.seances.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('academique.seances.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('academique.seances.destroy', itemToDelete.value.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('academique.seances.statut', itemToDelete.value.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
...
const page = usePage();
// Make seances accessible in template from props or page
const seances = props.seances || page.props.seances;
// Real-time search with debounce
watch(
  () => searchFilters.value,
  () => {
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('seances-create')">
                        <Link :href="route('academique.seances.create')" class="btn btn-primary">
                            {{ t('actions.add') }}
                        </Link>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <div class="table-wrapper">
                            ...
                            <table class="custom-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ t('common.cours') || 'Cours' }}</th>
                                        <th>{{ t('common.date') || 'Date Seance' }}</th>
                                        <th>{{ t('common.heure_debut') || 'Heure Début' }}</th>
                                        <th>{{ t('common.heure_fin') || 'Heure Fin' }}</th>
                                        <th>{{ t('fields.status') || 'Statut' }}</th>
                                        <th class="fit">{{ t('common.actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <template v-if="seances?.data && seances?.data.length > 0">
                                        <tr v-for="item in seances?.data" :key="item.id">
                                            <td>{{ item.titre || '-' }}</td>
                                            <td>{{ item.cours || '-' }}</td>
                                            <td>{{ item.date || '-' }}</td>
                                            ...
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('academique.seances.show', item.id)" class="btn btn-secondary" :title="t('actions.view')">
                                                        <span class="fa fa-eye"></span>
                                                    </Link>
                                                    <Link :href="route('academique.seances.edit', item.id)" class="btn btn-primary" :title="t('actions.edit')">
                        ...
                        <div class="mobile-card-list">
                            <template v-if="seances?.data && seances?.data.length > 0">
                                <div v-for="item in seances?.data" :key="'m-' + item.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        ...
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('common.cours') || 'Cours' }}</span>
                                            <span class="mobile-card-value">{{ item.cours || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('common.date') || 'Date Seance' }}</span>
                                            <span class="mobile-card-value">{{ item.date || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('common.heure_debut') || 'Heure Début' }}</span>
                                            <span class="mobile-card-value">{{ item.heure_debut || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('common.heure_fin') || 'Heure Fin' }}</span>
                                            <span class="mobile-card-value">{{ item.heure_fin || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                        ...
                                        <div class="mobile-card-actions">
                                            <Link :href="route('academique.seances.show', item.id)" class="btn btn-secondary btn-sm" :title="t('actions.view')">
                                                <span class="fa fa-eye"></span>
                                            </Link>
                                            <Link :href="route('academique.seances.edit', item.id)" class="btn btn-primary btn-sm" :title="t('actions.edit')">
                                                <span class="fa fa-edit"></span>
                                            </Link>
            ...
            <!-- Pagination -->
            <Pagination :data="seances" :preserve-scroll="true" />
        </div>
        <!-- Confirmation Modal -->
        <ConfirmModal
