...
const search = () => {
    router.get(route('parametrage.categories_enseignant.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('parametrage.categories_enseignant.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('parametrage.categories_enseignant.destroy', itemToDelete.value?.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.categories_enseignant.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
...
<template>
    ...
    <div class="body-wrapper">
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('categories_enseignants-create')">
                        <Link :href="route('parametrage.categories_enseignant.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="categories_enseignants?.data && categories_enseignants?.data.length > 0">
                                        <tr v-for="categorie in categories_enseignants?.data" :key="categorie.id">
                                            ...
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link
                                                        :href="route('parametrage.categories_enseignant.show', categorie?.id)"
                                                    ...
                                                    <Link
                                                        v-if="can('categories_enseignants-edit')"
                                                        :href="route('parametrage.categories_enseignant.edit', categorie?.id)"
                                                    ...
                                                    <button
                                                        v-if="categorie.etat === 'actif' || categorie.statut === 'actif'"
                                                        @click="confirmDeactivate(categorie)"
                                                        class="btn btn-danger"
                                                    ...
                                                    <button
                                                        v-else
                                                        @click="confirmActivate(categorie)"
                        ...
                        <div class="mobile-card-list">
                            <template v-if="categories_enseignants?.data && categories_enseignants?.data.length > 0">
                                <div v-for="categorie in categories_enseignants?.data" :key="'m-' + categorie.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        ...
                                        <div class="mobile-card-row">
                                            ...
                                            <span class="mobile-card-value"><span class="badge" :class="categorie.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + categorie.etat) }}</span></span>
                                        </div>
                                    </div>
                                    <div class="mobile-card-actions">
                                        <Link :href="route('parametrage.categories_enseignant.show', categorie?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link v-if="can('categories_enseignants-edit')" :href="route('parametrage.categories_enseignant.edit', categorie?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                                        <button @click="confirmDelete(categorie)" class="btn btn-danger"><span class="fa fa-trash"></span></button>
                                        <button v-if="categorie.etat === 'actif' || categorie.statut === 'actif'" @click="confirmDeactivate(categorie)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
                                        <button v-else @click="confirmActivate(categorie)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                    </div>
                                </div>
                            </template>
                        ...
                        <!-- Pagination -->
                        <Pagination :data="categories_enseignants" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <ConfirmModal
            ...
            :message="deleteMode ? (t('messages.confirm.delete.message') || 'Êtes-vous sûr de vouloir supprimer cet élément ?') : (deactivateMode || activateMode) ? (categorieToDelete?.etat === 'actif' ? t('messages.confirm.deactivate.message') || 'Êtes-vous sûr de vouloir désactiver cet élément ?' : t('messages.confirm.activate.message') || 'Êtes-vous sûr de vouloir activer cet élément ?') : ''"
            :sub-message="deleteMode ? (t('messages.confirm.delete.warning') || 'Cette action ne peut pas être annulée.') : (deactivateMode || activateMode) ? (categorieToDelete?.etat === 'actif' ? t('messages.confirm.deactivate.warning') : t('messages.confirm.activate.warning')) : ''"
