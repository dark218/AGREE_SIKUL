...
const search = () => {
    router.get(route('parametrage.titres_civilites.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('parametrage.titres_civilites.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('parametrage.titres_civilites.destroy', itemToDelete.value?.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.titres_civilites.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('titres_civilites-create')">
                        <Link :href="route('parametrage.titres_civilites.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="props.titreCivilites?.data && props.titreCivilites?.data.length > 0">
                                        <tr v-for="titre in props.titreCivilites?.data" :key="titre.id">
                                            <td>{{ titre.code || '' }}</td>
                                            <td>{{ titre.libelle || '' }}</td>
                                            <td>{{ titre.sigle || '' }}</td>
                                            <td><span class="badge" :class="titre.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + titre.etat) }}</span></td>
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link
                                                        :href="route('parametrage.titres_civilites.show', titre?.id)"
                                                    ...
                                                    <Link
                                                        v-if="can('titres_civilites-edit')"
                                                        :href="route('parametrage.titres_civilites.edit', titre?.id)"
                                                    ...
                                                    <button
                                                        @click="confirmDelete(titre)"
                                                        class="btn btn-danger"
                                                        :title="t('actions.delete')"
                                                    >
                                                        <span class="fa fa-trash"></span>
                                                    </button>
                                                    <button v-if="titre.etat === 'actif' || titre.statut === 'actif'" @click="confirmDeactivate(titre)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
                                                    <button v-else @click="confirmActivate(titre)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                                </div>
                                            </td>
                                        </tr>
                                    </template>
                        ...
                        <div class="mobile-card-list">
                            <template v-if="props.titreCivilites?.data && props.titreCivilites?.data.length > 0">
                                <div v-for="titre in props.titreCivilites?.data" :key="'m-' + titre.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.code') || 'Code' }}</span>
                                            <span class="mobile-card-value">{{ titre.code || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.label') || 'Libellé' }}</span>
                                            <span class="mobile-card-value">{{ titre.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.sigle') || 'Sigle' }}</span>
                                            <span class="mobile-card-value">{{ titre.sigle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.status') || 'Statut' }}</span>
                                            <span class="mobile-card-value"><span class="badge" :class="titre.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + titre.etat) }}</span></span>
                                        </div>
                                    </div>
                                    <div class="mobile-card-actions">
                                        <Link :href="route('parametrage.titres_civilites.show', titre?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link v-if="can('titres_civilites-edit')" :href="route('parametrage.titres_civilites.edit', titre?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                                        <button @click="confirmDelete(titre)" class="btn btn-danger"><span class="fa fa-trash"></span></button>
                                        <button v-if="titre.etat === 'actif' || titre.statut === 'actif'" @click="confirmDeactivate(titre)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
                                        <button v-else @click="confirmActivate(titre)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                    </div>
                                </div>
                        ...
                        <!-- Pagination -->
                        <Pagination :data="props.titreCivilites" />
                    </div>
                </div>
            </div>
        </div>
