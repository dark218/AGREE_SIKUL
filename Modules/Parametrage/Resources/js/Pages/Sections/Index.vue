...
const search = () => {
    router.get(route('parametrage.sections.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('parametrage.sections.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('parametrage.sections.destroy', itemToDelete.value?.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.sections.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('sections-create')">
                        <Link :href="route('parametrage.sections.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="sections?.data && sections?.data.length > 0">
                                        <tr v-for="section in sections?.data" :key="section.id">
                                            ...
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link
                                                        :href="route('parametrage.sections.show', section?.id)"
                                                    ...
                                                    <Link
                                                        ...
                                                        :href="route('parametrage.sections.edit', section?.id)"
                                                    ...
                                                    <button v-if="section.etat === 'actif' || section.statut === 'actif'" @click="confirmDeactivate(section)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
                                                    <button v-else @click="confirmActivate(section)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                                </div>
                                            </td>
                                        </tr>
                                    </template>
                        ...
                        <div class="mobile-card-list">
                            <template v-if="sections?.data && sections?.data.length > 0">
                                <div v-for="section in sections?.data" :key="'m-' + section.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.code') || 'Code' }}</span>
                                            <span class="mobile-card-value">{{ section.code || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                        ...
                                        <div class="mobile-card-row">
                                            ...
                                            <span class="mobile-card-value"><span class="badge" :class="section.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ section.etat === 'actif' ? 'Actif' : 'Inactif' }}</span></span>
                                        </div>
                                    </div>
                                    <div class="mobile-card-actions">
                                        <Link :href="route('parametrage.sections.show', section?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link v-if="can('sections-edit')" :href="route('parametrage.sections.edit', section?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                                        <button @click="confirmDelete(section)" class="btn btn-danger"><span class="fa fa-trash"></span></button>
                                        <button v-if="section.etat === 'actif' || section.statut === 'actif'" @click="confirmDeactivate(section)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
                                        <button v-else @click="confirmActivate(section)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                    </div>
                                </div>
