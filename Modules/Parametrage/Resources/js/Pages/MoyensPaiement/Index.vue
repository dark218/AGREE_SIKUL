...
const page = usePage();
const moyens_paiement = computed(() => page.props.moyens_paiement);
...
const search = () => {
    router.get(route('parametrage.moyens_paiement.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('parametrage.moyens_paiement.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('parametrage.moyens_paiement.destroy', itemToDelete.value?.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.moyens_paiement.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('moyens_paiement-create')">
                        <Link :href="route('parametrage.moyens_paiement.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="moyens_paiement?.data && moyens_paiement?.data.length > 0">
                                        <tr v-for="mode in moyens_paiement?.data" :key="mode.id">
                                            <td>{{ mode.code || '' }}</td>
                                            <td>{{ mode.libelle || '' }}</td>
                                            <td><span class="badge" :class="mode.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + mode.etat) }}</span></td>
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <!-- Button 1: Voir (View) -->
                                                    <Link
                                                        :href="route('parametrage.moyens_paiement.show', mode?.id)"
                                                    ...
                                                    <Link
                                                        v-if="can('moyens_paiement-edit')"
                                                        :href="route('parametrage.moyens_paiement.edit', mode?.id)"
                                                    ...
                                                    <button
                                                        v-if="mode.etat === 'actif' || mode.statut === 'actif'"
                                                        @click="confirmDeactivate(mode)"
                                                        class="btn btn-danger"
                        ...
                        <div class="mobile-card-list">
                            <template v-if="moyens_paiement?.data && moyens_paiement?.data.length > 0">
                                <div v-for="mode in moyens_paiement?.data" :key="'m-' + mode.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.code') || 'Code' }}</span>
                                            <span class="mobile-card-value">{{ mode.code || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.label') || 'Libellé' }}</span>
                                            <span class="mobile-card-value">{{ mode.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.status') || 'Statut' }}</span>
                                            <span class="mobile-card-value"><span class="badge" :class="mode.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + mode.etat) }}</span></span>
                                        </div>
                                    </div>
                                    <div class="mobile-card-actions">
                                        <Link :href="route('parametrage.moyens_paiement.show', mode?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link v-if="can('moyens_paiement-edit')" :href="route('parametrage.moyens_paiement.edit', mode?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                                        <button @click="confirmDelete(mode)" class="btn btn-danger"><span class="fa fa-trash"></span></button>
                                        <button v-if="mode.etat === 'actif' || mode.statut === 'actif'" @click="confirmDeactivate(mode)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
                                        <button v-else @click="confirmActivate(mode)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                    </div>
                                </div>
                            </template>
                        ...
                        <!-- Pagination -->
                        <Pagination :data="moyens_paiement" />
                    </div>
                </div>
            </div>
        </div>
