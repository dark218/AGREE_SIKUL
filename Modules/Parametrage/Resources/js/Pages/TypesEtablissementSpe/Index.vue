...
const search = () => {
    router.get(route('parametrage.types_etablissement_spe.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const resetFilters = () => {
  ...
  router.get(route('parametrage.types_etablissement_spe.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('parametrage.types_etablissement_spe.destroy', itemToDelete.value?.id), {
            method: 'delete',
            preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.types_etablissement_spe.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('types_etablissement_spe-create')">
                        <Link :href="route('parametrage.types_etablissement_spe.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="typesEtablissementsSpe?.data && typesEtablissementsSpe?.data.length > 0">
                                        <tr v-for="item in typesEtablissementsSpe?.data" :key="item.id">
                                            <td>{{ item.code || '' }}</td>
                                            <td>{{ item.libelle || '' }}</td>
                                            <td><span class="badge" :class="item.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + item.etat) }}</span></td>
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('parametrage.types_etablissement_spe.show', item?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                                    <Link :href="route('parametrage.types_etablissement_spe.edit', item?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                        ...
                        <div class="mobile-card-list">
                            <template v-if="typesEtablissementsSpe?.data && typesEtablissementsSpe?.data.length > 0">
                                <div v-for="item in typesEtablissementsSpe?.data" :key="'m-' + item.id" class="mobile-card">
                        ...
                        <!-- Pagination -->
                        <Pagination :data="typesEtablissementsSpe" />
                    </div>
                </div>
            </div>
        </div>
