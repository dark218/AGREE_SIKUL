...
const search = () => {
    router.get(route('parametrage.fichiers.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('parametrage.fichiers.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('parametrage.fichiers.destroy', itemToDelete.value?.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.fichiers.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('fichiers-create')">
                        <Link :href="route('parametrage.fichiers.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="fichiers?.data && fichiers?.data.length > 0">
                                        <tr v-for="fichier in fichiers?.data" :key="fichier.id">
                                            <td>{{ fichier.code || '' }}</td>
                                            <td>{{ fichier.libelle || '' }}</td>
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('parametrage.fichiers.show', fichier?.id)" class="btn btn-secondary">
                                                        <span class="fa fa-eye"></span>
                                                    </Link>
                                                    <Link :href="route('parametrage.fichiers.edit', fichier?.id)" class="btn btn-primary">
                                                        <span class="fa fa-edit"></span>
                                                    </Link>
                                                    <button @click="confirmDelete(fichier)" class="btn btn-danger">
                                                        <span class="fa fa-trash"></span>
                                                    </button>
                                                    <button v-if="fichier.etat === 'actif' || fichier.statut === 'actif'"
                                                        @click="confirmDeactivate(fichier)" class="btn btn-danger">
                                                        <span class="fa fa-ban"></span>
                                                    </button>
                                                    <button v-else @click="confirmActivate(fichier)" class="btn btn-success">
                        ...
                        <div class="mobile-card-list">
                            <template v-if="fichiers?.data && fichiers?.data.length > 0">
                                <div v-for="fichier in fichiers?.data" :key="'m-' + fichier.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.code') || 'Code' }}</span>
                                            <span class="mobile-card-value">{{ fichier.code || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.label') || 'Libellé' }}</span>
                                            <span class="mobile-card-value">{{ fichier.libelle || '-' }}</span>
                                        </div>
                                    </div>
                                    <div class="mobile-card-actions">
                                        <Link :href="route('parametrage.fichiers.show', fichier?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link :href="route('parametrage.fichiers.edit', fichier?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                                        <button @click="confirmDelete(fichier)" class="btn btn-danger"><span class="fa fa-trash"></span></button>
                                        <button v-if="fichier.etat === 'actif' || fichier.statut === 'actif'"
                                            @click="confirmDeactivate(fichier)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
                                        <button v-else @click="confirmActivate(fichier)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                    </div>
                                </div>
                            </template>
