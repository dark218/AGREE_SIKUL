...
const search = () => {
    router.get(route('parametrage.communes.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('parametrage.communes.index'));
};
...
const deleteItem = () => {
    ...
    if (itemToDelete.value) {
        console.log('🟢 itemToDelete exists:', itemToDelete.value);
        console.log('🟠 Deleting commune ID:', itemToDelete.value.id);
        console.log('🟡 URL will be:', route('parametrage.communes.destroy', itemToDelete.value?.id));
        showDeleteLoader();
        router.visit(route('parametrage.communes.destroy', itemToDelete.value?.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
            ...
            onError: (errors) => {
                console.error('❌ Delete onError fired:', errors);
            },
            onFinish: () => {
                console.log('🔵 Delete onFinish fired');
                hideLoader();
            }, });
    } else {
        console.error('❌ itemToDelete is null/undefined');
    }
};
const confirmDeactivate = (item) => {
    console.log('🟣 confirmDeactivate called with item:', item);
    itemToDelete.value = item;
    deactivateMode.value = true;
    deleteMode.value = false;
    showDeleteModal.value = true;
    console.log('🟣 Modal state set - showDeleteModal:', showDeleteModal.value);
};
const confirmActivate = (item) => {
    console.log('🟡 confirmActivate called with item:', item);
    itemToDelete.value = item;
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.communes.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('communes-create')">
                        <Link :href="route('parametrage.communes.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="communes?.data && communes?.data.length > 0">
                                        <tr v-for="commune in communes?.data" :key="commune.id">
                                            <td>{{ commune.code || '' }}</td>
                                            <td>{{ commune.libelle || '' }}</td>
                                            <td>{{ commune.departement?.libelle || '-' }}</td>
                                            <td>{{ commune.region?.libelle || '-' }}</td>
                                            <td>{{ commune.pays?.libelle || '-' }}</td>
                                            <td><span class="badge" :class="commune.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + commune.etat) }}</span></td>
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <!-- Button 1: Voir (View) -->
                                                    <Link
                                                        :href="route('parametrage.communes.show', commune?.id)"
                                                    ...
                                                    <Link
                                                        v-if="can('communes-edit')"
                                                        :href="route('parametrage.communes.edit', commune?.id)"
                                                    ...
                                                    <button
                                                        v-if="commune.etat === 'actif' || commune.statut === 'actif'"
                                                        @click="confirmDeactivate(commune)"
                                                        class="btn btn-danger"
                        ...
                        <div class="mobile-card-list">
                            <template v-if="communes?.data && communes?.data.length > 0">
                                <div v-for="commune in communes?.data" :key="'m-' + commune.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.code') || 'Code' }}</span>
                                            <span class="mobile-card-value">{{ commune.code || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.label') || 'Libellé' }}</span>
                                            <span class="mobile-card-value">{{ commune.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.departement') || 'Département' }}</span>
                                            <span class="mobile-card-value">{{ commune.departement?.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.region') || 'Région' }}</span>
                                            <span class="mobile-card-value">{{ commune.region?.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.country') || 'Pays' }}</span>
                                            <span class="mobile-card-value">{{ commune.pays?.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.status') || 'Statut' }}</span>
                                            <span class="mobile-card-value"><span class="badge" :class="commune.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + commune.etat) }}</span></span>
                                        </div>
                                    </div>
                                    <div class="mobile-card-actions">
                                        <Link :href="route('parametrage.communes.show', commune?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link v-if="can('communes-edit')" :href="route('parametrage.communes.edit', commune?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                                        <button @click="confirmDelete(commune)" class="btn btn-danger"><span class="fa fa-trash"></span></button>
                                        <button v-if="commune.etat === 'actif' || commune.statut === 'actif'" @click="confirmDeactivate(commune)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
                                        <button v-else @click="confirmActivate(commune)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                    </div>
                                </div>
                            </template>
