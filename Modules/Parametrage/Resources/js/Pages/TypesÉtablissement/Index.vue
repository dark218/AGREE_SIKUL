...
const search = () => {
    router.get(route('parametrage.types_etablissements.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('parametrage.types_etablissements.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('parametrage.types_etablissements.destroy', itemToDelete.value?.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.types_etablissements.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
...
<template>
    ...
    <div class="body-wrapper">
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('types_etablissements-create')">
                        <Link :href="route('parametrage.types_etablissements.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="typeEtablissements?.data && typeEtablissements?.data.length > 0">
                                        <tr v-for="item in typeEtablissements?.data" :key="item.id">
                                            <td>{{ item.code || '' }}</td>
                                            <td>{{ item.libelle || '' }}</td>
                                            <td><span class="badge" :class="item.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + item.etat) }}</span></td>
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('parametrage.types_etablissements.show', item?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                                    <Link :href="route('parametrage.types_etablissements.edit', item?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                        ...
                        <div class="mobile-card-list">
                            <template v-if="typeEtablissements?.data && typeEtablissements?.data.length > 0">
                                <div v-for="item in typeEtablissements?.data" :key="'m-' + item.id" class="mobile-card">
                        ...
                        <!-- Pagination -->
                        <Pagination :data="typeEtablissements" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <ConfirmModal
            ...
            @close="closeModal"
            @confirm="deleteMode ? deleteTypeEtablissement() : toggleStatus()"
        />
        <!-- Loader pleine page -->
        <FullPageLoader
            :show="isLoading"
