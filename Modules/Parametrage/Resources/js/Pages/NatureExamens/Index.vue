...
const search = () => {
    router.get(route('parametrage.natures_examens.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('parametrage.natures_examens.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('parametrage.natures_examens.destroy', itemToDelete.value?.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.natures_examens.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('natures-examens-create')">
                        <Link :href="route('parametrage.natures_examens.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="natureExamens?.data && natureExamens?.data.length > 0">
                                        <tr v-for="item in natureExamens?.data" :key="item.id">
                                            ...
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link :href="route('parametrage.natures_examens.show', item?.id)" class="btn btn-secondary" :title="t('actions.view')">
                                                        <span class="fa fa-eye"></span>
                                                    </Link>
                                                    <Link :href="route('parametrage.natures_examens.edit', item?.id)" class="btn btn-primary" :title="t('actions.edit')">
                                                        <span class="fa fa-edit"></span>
                                                    </Link>
                                                    <button @click="confirmDelete(item)" class="btn btn-danger" :title="t('actions.delete')">
                                                        <span class="fa fa-trash"></span>
                                                    </button>
                                                    <button v-if="item.etat === 'actif' || item.statut === 'actif'" @click="confirmDeactivate(item)" class="btn btn-danger" :title="t('actions.deactivate')">
                        ...
                        <div class="mobile-card-list">
                            <template v-if="natureExamens?.data && natureExamens?.data.length > 0">
                                <div v-for="item in natureExamens?.data" :key="'m-' + item.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        ...
                                        <div class="mobile-card-row">
                                            ...
                                            <span class="mobile-card-value">{{ item.section?.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.niveau') || 'Niveau' }}</span>
                                            <span class="mobile-card-value">{{ item.niveau?.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.cycle') || 'Cycle' }}</span>
                                            <span class="mobile-card-value">{{ item.cycle?.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.poids') || 'Poids' }}</span>
                                            <span class="mobile-card-value">{{ item.poids || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.pays') || 'Pays' }}</span>
                                            <span class="mobile-card-value">{{ item.pays?.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                    ...
                                    <div class="mobile-card-actions">
                                        <Link :href="route('parametrage.natures_examens.show', item?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link :href="route('parametrage.natures_examens.edit', item?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                                        <button @click="confirmDelete(item)" class="btn btn-danger"><span class="fa fa-trash"></span></button>
                                        <button v-if="item.etat === 'actif' || item.statut === 'actif'" @click="confirmDeactivate(item)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
