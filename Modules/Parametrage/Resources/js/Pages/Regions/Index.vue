...
const search = () => {
    router.get(route('parametrage.regions.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('parametrage.regions.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('parametrage.regions.destroy', itemToDelete.value?.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.regions.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
...
const page = usePage();
// Make regions accessible in template from props or page
const regions = props.regions || page.props.regions;
// Real-time search with debounce
watch(
  () => searchFilters.value,
  () => {
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('regions-create')">
                        <Link :href="route('parametrage.regions.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="regions?.data && regions?.data.length > 0">
                                        <tr v-for="region in regions?.data" :key="region.id">
                                            <td>{{ region.code || '' }}</td>
                                            <td>{{ region.libelle || '' }}</td>
                                            <td>{{ region.pays?.libelle || '-' }}</td>
                                            <td><span class="badge" :class="region.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + region.etat) }}</span></td>
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <!-- Button 1: Voir (View) -->
                                                    <Link
                                                        :href="route('parametrage.regions.show', region?.id)"
                                                    ...
                                                    <Link
                                                        v-if="can('regions-edit')"
                                                        :href="route('parametrage.regions.edit', region?.id)"
                        ...
                        <div class="mobile-card-list">
                            <template v-if="regions?.data && regions?.data.length > 0">
                                <div v-for="region in regions?.data" :key="'m-' + region.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.code') || 'Code' }}</span>
                                            <span class="mobile-card-value">{{ region.code || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.label') || 'Libellé' }}</span>
                                            <span class="mobile-card-value">{{ region.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.country') || 'Pays' }}</span>
                                            <span class="mobile-card-value">{{ region.pays?.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.status') || 'Statut' }}</span>
                                            <span class="mobile-card-value"><span class="badge" :class="region.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + region.etat) }}</span></span>
                                        </div>
                                    </div>
                                    <div class="mobile-card-actions">
                                        <Link :href="route('parametrage.regions.show', region?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link v-if="can('regions-edit')" :href="route('parametrage.regions.edit', region?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                                        <button @click="confirmDelete(region)" class="btn btn-danger"><span class="fa fa-trash"></span></button>
                                        <button v-if="region.etat === 'actif' || region.statut === 'actif'" @click="confirmDeactivate(region)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
                                        <button v-else @click="confirmActivate(region)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                    </div>
                                </div>
                            </template>
