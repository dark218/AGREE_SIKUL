...
const search = () => {
    router.get(route('parametrage.cycles_enseignement.index'), searchFilters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
const resetFilters = () => {
  ...
  router.get(route('parametrage.cycles_enseignement.index'));
};
...
const deleteItem = () => {
    if (itemToDelete.value) {
        showDeleteLoader();
        router.visit(route('parametrage.cycles_enseignement.destroy', itemToDelete.value?.id), { method: 'delete', preserveScroll: true,
            onSuccess: () => {
...
const toggleStatus = () => {
    if (itemToDelete.value) {
        ...
        router.visit(route('parametrage.cycles_enseignement.statut', itemToDelete.value?.id), { method: 'put',
            preserveScroll: true,
            onSuccess: () => {
                showDeleteModal.value = false;
                itemToDelete.value = null;
        ...
        <div class="table-area">
            ...
            <div class="dashboard-header-wrapper">
                ...
                <div class="dashboard-btn-wrapper">
                    <div class="dashboard-btn" v-if="can('cycles_enseignement-create')">
                        <Link :href="route('parametrage.cycles_enseignement.create')" class="btn btn-primary">
                    ...
                    <div class="table-wrapper">
                        <div class="table-responsive">
                            <table class="custom-table">
                                ...
                                <tbody>
                                    <template v-if="cycles?.data && cycles?.data.length > 0">
                                        <tr v-for="cycle in cycles?.data" :key="cycle.id">
                                            <td>{{ cycle.code || '' }}</td>
                                            <td>{{ cycle.libelle || '' }}</td>
                                            <td>{{ cycle.pays?.libelle || '-' }}</td>
                                            <td><span class="badge" :class="cycle.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + cycle.etat) }}</span></td>
                                            <td class="fit">
                                                <div class="action-buttons">
                                                    <Link
                                                        :href="route('parametrage.cycles_enseignement.show', cycle?.id)"
                                                    ...
                                                    <Link
                                                        v-if="can('cycles_enseignement-edit')"
                                                        :href="route('parametrage.cycles_enseignement.edit', cycle?.id)"
                                                    ...
                                                    <button
                                                        v-if="cycle.etat === 'actif' || cycle.statut === 'actif'"
                                                        @click="confirmDeactivate(cycle)"
                                                        class="btn btn-danger"
                        ...
                        <div class="mobile-card-list">
                            <template v-if="cycles?.data && cycles?.data.length > 0">
                                <div v-for="cycle in cycles?.data" :key="'m-' + cycle.id" class="mobile-card">
                                    <div class="mobile-card-body">
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.code') || 'Code' }}</span>
                                            <span class="mobile-card-value">{{ cycle.code || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.label') || 'Libellé' }}</span>
                                            <span class="mobile-card-value">{{ cycle.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.pays') || 'Pays' }}</span>
                                            <span class="mobile-card-value">{{ cycle.pays?.libelle || '-' }}</span>
                                        </div>
                                        <div class="mobile-card-row">
                                            <span class="mobile-card-label">{{ t('fields.status') || 'Statut' }}</span>
                                            <span class="mobile-card-value"><span class="badge" :class="cycle.etat === 'actif' ? 'bg-success' : 'bg-danger'">{{ t('common.' + cycle.etat) }}</span></span>
                                        </div>
                                    </div>
                                    <div class="mobile-card-actions">
                                        <Link :href="route('parametrage.cycles_enseignement.show', cycle?.id)" class="btn btn-secondary"><span class="fa fa-eye"></span></Link>
                                        <Link v-if="can('cycles_enseignement-edit')" :href="route('parametrage.cycles_enseignement.edit', cycle?.id)" class="btn btn-primary"><span class="fa fa-edit"></span></Link>
                                        <button @click="confirmDelete(cycle)" class="btn btn-danger"><span class="fa fa-trash"></span></button>
                                        <button v-if="cycle.etat === 'actif' || cycle.statut === 'actif'" @click="confirmDeactivate(cycle)" class="btn btn-danger"><span class="fa fa-ban"></span></button>
                                        <button v-else @click="confirmActivate(cycle)" class="btn btn-success"><span class="fa fa-check"></span></button>
                                    </div>
                                </div>
                            </template>
