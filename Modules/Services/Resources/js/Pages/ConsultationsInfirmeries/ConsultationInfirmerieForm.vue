<template>
    <form @submit.prevent="submit" class="consultation-infirmerie-form">
        ...
        <div v-if="!isReadOnly" class="form-actions mt-4">
            ...
            <Link :href="route('consultation-infirmerie.index')" class="btn btn-secondary ms-2">
                {{ t('common.cancel') }}
            </Link>
        </div>
    </form>
...
const form = ref({
    nom: props.consultationInfirmerie?.nom || '',
    code: props.consultationInfirmerie?.code || '',
    statut: props.consultationInfirmerie?.statut || 'actif',
});
function submit() {
