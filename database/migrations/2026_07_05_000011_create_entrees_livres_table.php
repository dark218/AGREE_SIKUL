...
/**
 * Entrées de livres (emprunt / achat / don) dans une bibliothèque.
 */
return new class extends Migration
{
    public function up(): void
    {
        if (Schema::hasTable('entrees_livres')) {
            return;
        }

        Schema::create('entrees_livres', function (Blueprint $table) {
            $table->id();
            ...
            $table->enum('type_entree', ['emprunt', 'achat', 'don'])->default('achat');
            $table->date('date_entree')->nullable();
            $table->unsignedInteger('quantite')->default(1);
            $table->date('date_retour')->nullable();
            $table->string('tiers', 255)->nullable(); // Prêteur / Vendeur / Donateur
    ...
    {
        Schema::dropIfExists('entrees_livres');
    }
};
